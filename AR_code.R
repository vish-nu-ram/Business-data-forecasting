...
valid_data_ts = ts(valid_data, start = 66, end = 80, frequency=7)
...
train.lm.trend.season.pred <- forecast(train.lm.trend.season, h = 80, level = 0)
...
tsdiag(train.res.ar)
train.res.ar.pred <- forecast(train.res.ar, h=7)
summary(train.res.ar)

plot(train.res.ar.pred)
lines(testing, col="red")

ts_forecast <- forecast(train.res.ar, h= length(valid_data), level=0)
plot(ts_forecast)
lines(ts_forecast$fitted, lwd = 1, col = "blue")
...
valid.res.ar.pred <- predict(train.res.ar, n.ahead = 14)
...
Acf(train.res.ar$residuals, lag.max = 12, main = "")
plot(train.lm.trend.season$residuals, xlim = c(1, length(train.lm.trend.season$residuals) + 14),
